...
export async function GET(
  ...
  try {
    ...
    const rows = students.map((student: StudentRow, index: number) => {
    ...
    // Add summary row
    const totalIncome = students.reduce((sum: number, s: StudentRow) => sum + Number(s.income || 0), 0);
    const totalExpense = students.reduce((sum: number, s: StudentRow) => sum + Number(s.expense || 0), 0);
