...
export default function DuelPage() {
  ...
  const getOptionStyle = (optionKey: string) => {
    ...
    if (!isDisabled) {
      return `${base} border-slate-200 hover:border-emerald-500 hover:bg-emerald-50/70 hover:shadow-xs cursor-pointer text-slate-800`;
    }
    if (optionKey === selectedOption) {
      return `${base} bg-emerald-600 border-emerald-600 text-white font-bold shadow-md`;
    }
  ...
  if (showResult && resultData) {
    ...
    return (
      <div className="space-y-6 max-w-md mx-auto">
        ...
        <button
          onClick={() => router.push("/murid/battle")}
          className="w-full py-3 bg-emerald-600 text-white rounded-xl hover:bg-emerald-700 transition-colors font-mono text-sm font-bold shadow-md cursor-pointer flex items-center justify-center gap-2"
        >
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-2xs">
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2 min-w-0 flex-1">
            <div className="w-9 h-9 rounded-full bg-emerald-100 flex items-center justify-center shrink-0 border border-emerald-200">
              {isTeam ? (
                <Users className="w-4 h-4 text-emerald-600" />
              ) : (
            ...
            <div className="space-y-1">
              ...
              {myTeamMembers.map((member, idx) => {
                ...
                return (
                  ...
                  >
                    {isActive && <Zap className="w-3 h-3 text-emerald-500 shrink-0" />}
                    <span className="truncate">{member.user_name}</span>
                    {member.user_id === match.userId && (
                      <span className="text-[9px] bg-emerald-100 text-emerald-600 px-1 rounded shrink-0">Kamu</span>
                    )}
                  </div>
                );
              })}
      ...
      {currentQuestion ? (
        <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-2xs space-y-5 relative overflow-hidden min-h-[320px]">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs font-mono text-emerald-600 bg-emerald-50 border border-emerald-100 px-2 py-0.5 rounded-md flex items-center gap-1">
