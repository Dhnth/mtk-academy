...
export default function BattlePage() {
      ...
      if (teamRes.ok) {
        ...
        if (teamData) {
          if (teamData.status === "MATCHMAKING") {
            window.location.href = `/murid/battle/team/${teamData.id}/match`;
            return;
          }
          if (teamData.status === "IN_BATTLE" && teamData.active_match_id) {
            window.location.href = `/murid/battle/duel/${teamData.active_match_id}`;
            return;
          }
        }
        setMyTeam(teamData);
    ...
    const subscribe = () => {
      try {
        ...
        channel.bind("matchmaking-started", () => {
          window.location.href = `/murid/battle/team/${myTeam.id}/match`;
        });
  ...
  const handleJoinTeam = async () => {
    ...
    try {
      ...
      setTeamCode("");
      router.push(`/murid/battle/team/${data.teamId}`);
    } catch {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-white border border-slate-200 rounded-2xl p-6 hover:shadow-md transition-shadow">
          ...
          <button
            onClick={() => router.push("/murid/battle/solo")}
            className="w-full py-2.5 bg-emerald-600 text-white rounded-xl hover:bg-emerald-700 transition-colors font-mono text-sm font-bold"
          >
            Cari Lawan
          </button>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-6 hover:shadow-md transition-shadow">
          ...
          {isInTeam ? (
            <div className="space-y-3">
              <div className="bg-slate-50 rounded-xl p-4 text-center">
                ...
                <p className="text-xs text-slate-500 mb-3">
                  {myTeam.members.length}/4 anggota • Kode: <span className="font-mono font-bold text-emerald-600">{myTeam.code}</span>
                </p>
                <button
                  onClick={() => router.push(`/murid/battle/team/${myTeam.id}`)}
                  className="w-full py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-sm font-mono font-bold transition-colors"
                >
                  Masuk Halaman Tim
                </button>
              </div>
            ...
            <div className="space-y-3">
              <button
                onClick={() => router.push("/murid/battle/team/create")}
                className="w-full py-2.5 bg-emerald-600 text-white rounded-xl hover:bg-emerald-700 transition-colors font-mono text-sm font-bold"
              >
                Buat Tim
              </button>
              <button
                ...
                className="w-full py-2.5 border-2 border-dashed border-emerald-300 text-emerald-600 rounded-xl hover:bg-emerald-50 transition-colors font-mono text-sm font-bold"
              >
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent text-sm font-mono uppercase"
                  maxLength={10}
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  className="flex-1 px-4 py-2.5 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition-colors font-mono text-sm font-bold disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 cursor-pointer"
                >
                  {joinLoading ? (
                    <>
