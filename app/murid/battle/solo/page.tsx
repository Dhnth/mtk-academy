...
export default function SoloMatchmakingPage() {
  ...
  useEffect(() => {
    ...
    const channel = (pusherClient as PusherClient).subscribe("arena-global");
    ...
    channel.bind("match-found", (data: unknown) => {
      ...
      fetch("/api/battle/profile")
        .then((res) => res.json())
        .then((profile) => {
          if (matchData.player1Id === profile.id || matchData.player2Id === profile.id) {
            setIsQueueing(false);
  ...
  const startQueue = async () => {
    ...
    try {
      ...
      const userChannel = (pusherClient as PusherClient).subscribe(`user-${data.userId}`);
      ...
      userChannel.bind("match-found", (matchData: unknown) => {
        const mData = matchData as UserMatchData;
        clearInterval(timerRef.current!);
        setIsQueueing(false);
        userChannel.unbind_all();
        userChannel.unsubscribe();
        window.location.href = `/murid/battle/duel/${mData.matchId}`;
      });
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-2xs">
        {isQueueing ? (
          ...
          <div className="text-center space-y-4">
            ...
            <button
              onClick={startQueue}
              className="w-full py-3 bg-emerald-600 text-white rounded-xl hover:bg-emerald-700 transition-colors font-mono text-sm font-bold"
            >
              Cari Lawan
            </button>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl w-full max-w-sm p-6 shadow-xl">
            ...
            <button
              ...
              className="w-full px-4 py-2.5 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition-colors font-mono text-sm font-bold"
            >
              Kembali ke Battle
            </button>
          </div>
