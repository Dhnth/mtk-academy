...
export default function TeamMatchmakingPage() {
      ...
      try {
        ...
        if (teamRes.ok) {
          ...
          if (teamData.status !== "MATCHMAKING") {
            window.location.href = `/sekretaris/battle/team/${teamId}`;
            return;
          }
          setTeam(teamData);
        } else {
    ...
    const subscribe = () => {
      try {
        ...
        channel.bind("team-disband", () => {
          if (timerRef.current) clearInterval(timerRef.current);
          window.location.href = `/sekretaris/battle?autoRefresh=1`;
        });
      } catch {
        // Error handling
      }
  ...
  const handleCancel = async () => {
    ...
    try {
      ...
      // Pusher will fire matchmaking-cancelled and redirect automatically
    } catch {
  ...
  return (
    <div className="space-y-6 max-w-md mx-auto">
      {/* Back Button */}
      <button
        onClick={() => router.push(`/sekretaris/battle/team/${teamId}`)}
      ...
      <div className="text-center">
        ...
        {team && (
          <p className="text-purple-700 font-bold text-base mt-1 font-mono">
            {team.name}
          </p>
        )}
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-8 shadow-2xs text-center space-y-6">
        ...
        <div className="relative flex items-center justify-center">
          <div className="absolute w-24 h-24 rounded-full bg-purple-100 animate-ping opacity-40" />
          <div className="absolute w-20 h-20 rounded-full bg-purple-200 animate-pulse opacity-60" />
          <div className="relative w-16 h-16 rounded-full bg-purple-600 flex items-center justify-center shadow-lg">
