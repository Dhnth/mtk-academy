...
export default function ProfileContent() {
  ...
  useEffect(() => {
    if (stats?.username) {
      fetch(`/api/profile/qr?username=${stats.username}`)
        .then((res) => res.blob())
        .then((blob) => {
  ...
  const downloadQR = () => {
    if (qrImageUrl) {
      ...
      link.download = `qr-${stats?.username}.png`;
    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-2xs">
        ...
        <div className="flex justify-center">
          {qrLoading ? (
            <div className="w-48 h-48 bg-slate-100 rounded-xl animate-pulse flex items-center justify-center">
              <span className="text-slate-400 text-sm font-mono">Loading...</span>
            </div>
          ) : qrImageUrl ? (
            <div ref={qrRef} className="p-3 bg-white border border-slate-200 rounded-xl">
              <img
                src={qrImageUrl}
                alt="QR Code Profile"
                className="w-48 h-48"
            ...
            <div className="w-48 h-48 bg-red-50 border border-red-200 rounded-xl flex items-center justify-center">
              <span className="text-red-500 text-sm font-mono">Gagal load QR</span>
            </div>
          )}
        </div>
        <p className="text-center text-[10px] text-slate-400 font-mono mt-3">
          Scan QR code untuk melihat profile publik
        </p>
      </div>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-2xs">
            ...
            <div className="mt-4">
              <div className="flex justify-between text-xs text-slate-500 mb-1">
                ...
                <span className="font-mono">{stats.exp} / {stats.level * 100}</span>
              </div>
